...
namespace VT
{
    public class RVTTerrain : MonoBehaviour
    {
        ...
        public Rect RealTotalRect
        {
            ...
            {
                realTotalRect           = value;
                Shader.SetGlobalVector("_VTWorldRect", new Vector4(realTotalRect.xMin, realTotalRect.yMin, realTotalRect.width, realTotalRect.height));
            }
        }
        [HideInInspector]
        ...
        /// <summary>
        /// mVTTileBuffer 关联 tileTexture 的超级纹理
        /// </summary>
            ...
            foreach (var ter in TerrainList)
            {
                ...
                foreach (var alphamap in ter.terrainData.alphamapTextures)
                {
                    ...
                    for (; layerIndex < ter.terrainData.terrainLayers.Length && index <= 4; layerIndex++)
                    {
                        ...
                        var nowScale        = new Vector2(ter.terrainData.size.x / layer.tileSize.x,ter.terrainData.size.z / layer.tileSize.y);
                        var tileOffset      = new Vector4(nowScale.x * scaleOffset.x,nowScale.y * scaleOffset.y, scaleOffset.z * nowScale.x, scaleOffset.w * nowScale.y);
        ...
        {
            var sector              = new Vector2Int((int)pos.x, (int)pos.y) -  new Vector2Int((int)realRect.xMin, (int)realRect.yMin);
